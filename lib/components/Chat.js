...
function () {
  function Chat(username, password, channel, obs) {
    ...
    this.obs = obs.obs;
    this.prefix = "!";
    this.commands = ["host", "unhost", "start", "stop", "switch", "raid", "bitrate", "info"];
  ...
  _createClass(Chat, [{
      ...
      if (message !== null) {
        ...
        if (parsed !== null) {
          if (parsed.command === "PRIVMSG") {
            ...
            if (_config.default.twitchChat.allowedUsers.includes(parsed.username) && this.rate != 20 || _config.default.twitchChat.enablePublicCommands && this.allowAllCommands.includes(commandName) && !this.wait && this.rate != 20 || parsed.username === this.username && this.rate != 20) {
    ...
    value: function info() {
      var bitrate = Math.round(this.obsProps.bitrate);
      this.ws.send("PRIVMSG ".concat(this.channel, " :Current scene: ").concat(this.obsProps.currentScene, " and bitrate: ").concat(bitrate));
    }
  }]);
