...
function () {
  function Chat(username, password, channel, obs) {
    ...
    this.obs = obs.obs;
    this.prefix = "!";
    this.commands = ["host", "unhost", "start", "stop", "switch", "raid", "bitrate", "status"];
  ...
  _createClass(Chat, [{
      ...
      if (message !== null) {
        ...
        if (parsed !== null) {
          if (parsed.command === "PRIVMSG") {
            ...
            console.log(parsed.username, _config.default.twitchChat.username);

            if (_config.default.twitchChat.allowedUsers.includes(parsed.username) && this.rate != 20 || _config.default.twitchChat.enablePublicCommands && this.allowAllCommands.includes(commandName) && !this.wait && this.rate != 20 || parsed.username === _config.default.twitchChat.username && this.rate != 20) {
    ...
    value: function status() {
      this.ws.send("PRIVMSG ".concat(this.channel, " :Current scene: ").concat(this.obsProps.currentScene));
    }
  }]);
