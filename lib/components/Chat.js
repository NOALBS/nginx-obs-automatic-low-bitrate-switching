...
function () {
  function Chat(username, password, channel, obs) {
    ...
    this.obs = obs.obs;
    this.prefix = "!";
    this.commands = ["host", "unhost", "start", "stop", "switch", "raid", "bitrate", "info", "refresh"];
  ...
  _createClass(Chat, [{
    ...
    value: function host(username) {
      ...
      if (username != null) {
        ...
        setTimeout(function () {
          _this3.stop();
        }, _config.default.twitchChat.stopStreamOnHostInterval);
      } else {
    ...
    value: function raid(username) {
      ...
      if (username != null) {
        ...
        setTimeout(function () {
          _this4.stop();
        }, _config.default.twitchChat.stopStreamOnRaidInterval);
      } else {
        console.log("Error executing host command no username");
        this.ws.send("PRIVMSG ".concat(this.channel, " :Error no username"));
      }
    ...
    value: function bitrate() {
      this.ws.send("PRIVMSG ".concat(this.channel, " :Current bitrate: ").concat(this.obsProps.bitrate));
    }
  }, {
    key: "info",
    value: function info() {
      this.ws.send("PRIVMSG ".concat(this.channel, " :Current scene: ").concat(this.obsProps.currentScene, " and bitrate: ").concat(this.obsProps.bitrate));
    }
  }, {
    key: "refresh",
    value: function () {
          ...
          while (1) {
            switch (_context4.prev = _context4.next) {
              ...
              case 4:
                ...
                setTimeout(function () {
                  _this5.obs.setCurrentScene({
                    "scene-name": _config.default.obs.normalScene
                  });

                  _this5.ws.send("PRIVMSG ".concat(_this5.channel, " :Refreshing stream completed"));
    ...
    key: "live",
    value: function live() {
      this.ws.send("PRIVMSG ".concat(this.channel, " :Stream went live"));
    }
  }, {
    key: "onNormalScene",
    ...
    value: function onOfflineScene() {
      this.ws.send("PRIVMSG ".concat(this.channel, " :Stream went offline"));
    }
  }]);
