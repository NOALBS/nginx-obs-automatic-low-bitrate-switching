...
function () {
  function Chat(username, password, channel, obs) {
    ...
    this.obs = obs.obs;
    this.prefix = "!";
    this.commands = ["host", "unhost", "start", "stop", "switch", "raid", "bitrate", "refresh", "trigger", "sourceinfo", "obsinfo", "public", "mod", "notify", "autostop"];
  ...
  _createClass(Chat, [{
    ...
    value: function sourceinfo() {
      if (this.obsProps.nginxVideoMeta != null) {
        ...
        this.ws.send("PRIVMSG ".concat(this.channel, " :[SRC] R: ").concat(height[0], " | F: ").concat(frame_rate[0], " | B: ").concat(this.obsProps.bitrate));
      } else {
        this.ws.send("PRIVMSG ".concat(this.channel, " :[Source Info] offline"));
      }
    }
  }, {
    key: "obsinfo",
    value: function obsinfo() {
      ...
      this.ws.send("PRIVMSG ".concat(this.channel, " :[OBS] S: ").concat(this.obsProps.currentScene, " | F: ").concat(Math.round(fps), " | B: ").concat(kbitsPerSec));
    }
  }, {
    key: "refresh",
    value: function () {
    ...
    key: "handleEnable",
    value: function handleEnable(name, message) {
      if (message === "on" && _config.default.twitchChat[name] != true) {
        _config.default.twitchChat[name] = true;
        this.handleWriteToConfig();
      } else if (message === "off" && _config.default.twitchChat[name] != false) {
